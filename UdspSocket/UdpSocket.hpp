...
public:
    ...
    // multicast:
    //   0 => no multicast
    //   224.0.0.0__239.255.255.255 => valid multicast range
    // interface:
    //   0 => all interfaces
    //  !0 => a specific interface (WiFi, Ethernet, etc)
    bool bind(const uint16_t port = 0, const IPAddress multicast = IPAddress(),
    ...
    bool setReuseAddress(const bool isEnabled);
