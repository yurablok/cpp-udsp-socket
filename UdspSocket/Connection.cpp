...
#         if UDSP_TRACE_LEVEL >= UDSP_TRACE_LEVEL_STATE_CHANGED
            std::cout << CLR_MAGENTA "Disconnected (closed)" CLR_RESET << std::endl;
#         endif // UDSP_TRACE_LEVEL
            ...
            if (c.lastPacketTick_us <= now_us - g_connectionTimeout_us) {
                ...
                c.onDisconnected();
#             if UDSP_TRACE_LEVEL >= UDSP_TRACE_LEVEL_STATE_CHANGED
                std::cout << CLR_MAGENTA "Disconnected (timeout)" CLR_RESET << std::endl;
#             endif // UDSP_TRACE_LEVEL
                if (onDisconnected != nullptr) {
                    onDisconnected(&c, 't');
                }
...
#     if UDSP_TRACE_LEVEL >= UDSP_TRACE_LEVEL_STATE_CHANGED
        std::cout << CLR_MAGENTA "Connected" CLR_RESET << std::endl;
#     endif // UDSP_TRACE_LEVEL
        if (onConnected) {
            onConnected(&c);
        }
