...
# if UDSP_TRACE_LEVEL >= UDSP_TRACE_LEVEL_TX_PACING \
        or UDSP_TRACE_LEVEL >= UDSP_TRACE_LEVEL_TX_KEEP_ALIVE
...
bool UDSPSocket::Connection::writePacket(
    ...
    while (true) {
        const uint32_t available_B = uint32_t(txBuffer.size() - offset_B);
        if (available_B == 0) {
            break;
        }
        const uint32_t written_B = writeMetaChunk(now_us, &txBuffer[offset_B], available_B);
    ...
    while (true) {
        ...
        const uint32_t written_B = writeDataChunk(now_us, &txBuffer[offset_B], available_B);
        assert(written_B <= available_B);
        if (written_B <= 0) {
            break;
        }
    ...
    if (RTTResponseTick_us != 0) {
        header.RTTDelay_us = uint32_t(now_us - RTTResponseTick_us);
    }
    else {
...
void UDSPSocket::Impl::processConnection(Connection& c, const int64_t now_us) {
    ...
    if (c.prevTick_us == 0) {
        ...
        // lim_B  1000000 us
        // max_B    diff_us
        maxPortion_B = uint32_t((diff_us * c.txLimit_B_s) / 1000000);
        c.prevTick_us = now_us;

        if (c.isRTTProbing) {
            constexpr uint32_t d = 1 << 8;
            constexpr uint32_t m = uint32_t(d * 0.65f); // 65 %
...
#             endif // UDSP_TRACE_LEVEL
    ...
    case PacketId::TypeA: {
        ...
        while (true) {
            const uint32_t available_B = size_B - uint32_t(offset_B);
            if (available_B == 0) {
                break;
            }
            const uint32_t readed_B = c.readChunk(now_us, &rxBuffer[offset_B], available_B);
    ...
    if (c.rxPacketsWindowIdx != rxPacketsWindowIdx) {
        ...
        if (rxPacketsCountInWindowExpected > 0) {
            if (c.rxPacketsCountInWindow < rxPacketsCountInWindowExpected) {
                c.rxPacketsLossInWindow_prc100 = uint16_t(10000
